...
const AnalystPage = () => {
  ...
  const totalViews = videos.reduce((sum, v) => sum + v[2], 0);
  const totalLikes = videos.reduce((sum, v) => sum + v[3], 0);
  const totalComments = videos.reduce((sum, v) => sum + v[4], 0);
  const totalShares = videos.reduce((sum, v) => sum + v[5], 0);
  ...
  const engagementData = [
    ...
    { metric: "Shares", value: totalShares, change: "+5.4%", icon: "📤", color: "purple" },
  ];

  const videoComparisonChartData = {
    labels: videos.map((item) => item[1]),
    datasets: [
      {
        label: "Views",
        data: videos.map((item) => item[2]),
        backgroundColor: [
          "rgba(59, 130, 246, 0.8)",
          "rgba(239, 68, 68, 0.8)",
          "rgba(34, 197, 94, 0.8)",
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-400">
                      <span>👁️ {video[2].toLocaleString()}</span>
                      <span>❤️ {video[3]}</span>
                      <span>💬 {video[4]}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-green-400 font-semibold text-sm">
                      {(((video[3] + video[4]) / video[2]) * 100).toFixed(1)}%
                    </div>
